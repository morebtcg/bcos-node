...
generate_config()
{
    ...
    if [ "${sm_mode}" == "false" ]; then
        generate_config_ini "${node_config_path}"
    else
        generate_sm_config_ini "${node_config_path}"
    fi
...
main() {
    ...
    for ((i = 0; i < node_count; i++)); do
        ...
        # node config
        generate_config "${sm_mode}" "${node_dir}/config.ini" "${node_dir}" "${connected_nodes}"
